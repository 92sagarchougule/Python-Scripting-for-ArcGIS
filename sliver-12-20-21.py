...
#get Parameters
arcpy.AddMessage("\n***getParameters")
Cluster_Tolerance=0.001 #arcpy.GetParameterAsText(3)  Tolerance
LandPortion=   #arcpy.GetParameterAsText(0)  Shapefile
RoadSections=  #arcpy.GetParameterAsText(1)  outputfile name
gdb=           #arcpy.GetParameterAsText(2)  Select Database
editRank="5"
fixRank="1"
...
arcpy.AddMessage("\n***add Rule Topology")
arcpy.AddRuleToTopology_management(gdb+os.path.sep+"topo"+os.path.sep+"coincidentBoundaries", ["Must Not Overlap (Area)","Must Not Have Gaps (Area)"], polyTopo, "", lineTopo, "")
arcpy.AddMessage("elapsed:\t"+str(datetime.datetime.now()-startTime))
